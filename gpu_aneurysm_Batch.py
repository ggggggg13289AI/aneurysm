...
def predict_vessel(image_arr, brain_mask,
    ...
    def sliding_window_inference1(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside", mask=None, batch_size=10, **kwargs):
        ...
        output_shape = tf.concat([[1], tf.shape(input_padded)[1:-1], [n_class]], axis=0)
        ...
        for start, size in window_slices:
            if mask is None or tf.reduce_any(tf.slice(mask_padded, begin=start, size=size)):
                valid_slices.append((start, size))
        ...
        for i, (start, size) in enumerate(valid_slices):
            ...
            if len(batch_inputs) == batch_size or i == len(window_slices) - 1:
                if not batch_inputs:
                    continue

                batch_tensor = tf.stack(batch_inputs, axis=0)  # shape: (B, D, H, W, C)
                #print(f"[Done combine batch] spend {time() - start_first:.0f} sec")
                #print('batch_tensor', batch_tensor.shape, 'importance_map:', importance_map.shape)
                preds = run_model1(batch_tensor, model, importance_map)  # shape: (B, D, H, W, n_class)

                for j, (s, sz) in enumerate(batch_indices):
                    pred = preds[j]  # shape: (D, H, W, n_class)

                    # 重新計算padding結構(完整處理5個維度)
                    padding = []
                    for dim in range(5):  # 處理所有維度(batch, D, H, W, channel)
                        if dim == 0 or dim == 4:  # batch和channel維度不填充
                            padding.append([0, 0])
                        else:  # 空間維度(D/H/W)
                            pad_after = output_shape[dim] - (s[dim] + sz[dim])
                            padding.append([
                                s[dim], 
                                tf.maximum(pad_after, 0)
                            ])
                    
                    # 驗證padding結構
                    assert len(padding) == 5, f"Invalid padding dimensions: {len(padding)}"
                    ...
                    try:
                        tf.debugging.assert_shapes([
                            (pred_padded, output_sum.shape),
                            (weight_padded, output_weight_sum.shape)
                        ])
                    except Exception as e:
                        print(f"Shape mismatch at batch index {j}:")
                        print(f"Pred padded: {pred_padded.shape}")
        ...
        #print('tf.reduce_sum(output_sum):', tf.reduce_sum(output_sum), ' tf.reduce_sum(output_weight_sum)', tf.reduce_sum(output_weight_sum))
        output = output_sum / tf.clip_by_value(output_weight_sum, 1, 256)
        #output = output_sum / tf.clip_by_value(output_weight_sum, 1e-8, tf.reduce_max(output_weight_sum))
        crop_slice = [slice(pad[0], pad[0] + input_x) for pad, input_x in zip(paddings[1:-1], inputs.shape[1:-1])]
    ...
    vessel_conf = sliding_window_inference1(inputs=image_arr[np.newaxis,:,:,:,np.newaxis],
                                            ...
                                            mask=brain_mask[np.newaxis,:,:,:,np.newaxis], batch_size=batch_size)
...
def run_model2(x, model2, importance_map, **kwargs):
    ...
    importance_map = tf.broadcast_to(importance_map, tf.shape(pred1))
    return pred1 * importance_map

#@title model predict aneurysm
def predict_aneurysm_ver1(image_arr, brain_mask, vessel_16labels, spacing,
                     model2, patch_size=(32, 32, 16), sigma=0.125, n_class=1, overlap=0.75,
                     conf_th=0.1, min_diameter=1, top_k=4, obj_th=0.67, batch_size=600, verbose=False):
    
    from time import time
    # 預處理
    
    if verbose: print(">>> Resampling inputs...")
    ...
    #print(f"[Done resize] spend {time() - start_resize:.0f} sec")

    def sliding_window_inference2(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside", mask=None, batch_size=10, verbose=False, **kwargs):
        image_size = tuple(inputs.shape[1:-1])
        roi_size = tuple(roi_size)

        # Padding to make sure image size is at least roi size
        padded_image_size = tuple(max(image_size[i], roi_size[i]) for i in range(3))
        padding_size = [image_x - input_x for image_x, input_x in zip(padded_image_size, image_size)]
        paddings = [[0, 0]] + [[x // 2, x - x // 2] for x in padding_size] + [[0, 0]]
        input_padded = tf.pad(inputs, paddings)
        ...
        for i, (start, size) in enumerate(valid_slices):
            ...
            if len(batch_inputs) == batch_size or i == len(window_slices) - 1:
                ...
                print(f"[Done combine batch] spend {time() - start_first:.0f} sec")
                print('batch_tensor', batch_tensor.shape, 'importance_map:', importance_map.shape)
                preds = run_model2(batch_tensor, model, importance_map)  # shape: (B, D, H, W, n_class)

                for j, (s, sz) in enumerate(batch_indices):
                    pred = preds[j]  # shape: (D, H, W, n_class)
                    pred_shape = tf.shape(pred)[0:3]
                    # 動態計算實際可裁剪範圍
                    # 提取空間維度(排除batch和channel)
                    spatial_sz = sz[1:-1]  # 原始sz包含batch和channel維度
                    spatial_pred_shape = tf.shape(pred)[0:3]
                    
                    # 動態計算實際可裁剪範圍
                    clipped_sz = tf.minimum(spatial_sz, spatial_pred_shape)
                    pred = tf.slice(pred, [0, 0, 0, 0], tf.concat([clipped_sz, [n_class]], axis=0))

                    # 重新計算pad_after避免負值
                    pad_before = s[1:]  # 排除batch維度
                    # 重新計算pad_after(僅處理空間維度)
                    spatial_s = s[1:-1]  # 排除batch和channel維度
                    pad_after = tf.maximum(
                        output_shape[1:-1] - spatial_s - clipped_sz, 
                        0
                    )
                    ...
                    pad = [
                        [0, 0],          # batch維度
                        [spatial_s[0], pad_after[0]],  # D維度
                        [spatial_s[1], pad_after[1]],  # H維度
                        [spatial_s[2], pad_after[2]],  # W維度
                        [0, 0]           # channel維度
                    ]

                    try:
                        # 使用tf.pad的constant_values參數明確指定填充值
                        pred_padded = tf.pad(pred[tf.newaxis], pad, constant_values=0)
                        weight_padded = tf.pad(importance_map, pad, constant_values=0)
                        
                        # 確保形狀完全匹配
                        output_sum = tf.ensure_shape(output_sum, pred_padded.shape)
                        output_weight_sum = tf.ensure_shape(output_weight_sum, weight_padded.shape)
                        
                        output_sum += pred_padded
                        output_weight_sum += weight_padded
                    except Exception as e:
                        print(f"[ERROR] Padding mismatch at index {j}: {e}")
                        print("pred shape:", tf.shape(pred).numpy())
                        print("pad:", pad)
                        continue
    ...
    if verbose: print(">>> Computing importance map...")
    importance_kernel = get_importance_kernel(patch_size, blend_mode="gaussian", sigma=sigma)
    importance_map = tf.tile(tf.reshape(importance_kernel, shape=[1, *patch_size, 1]), [1, 1, 1, 1, n_class])

    if verbose: print(">>> Running sliding window inference...")
    pred_prob_map = sliding_window_inference2(
        inputs=custom_normalize_1(image_iso)[np.newaxis, ..., np.newaxis],
        roi_size=patch_size, model=model2, overlap=overlap, n_class=n_class,
        importance_map=importance_map, mask=vessel_iso[np.newaxis, ..., np.newaxis],
    ...
    if verbose: print(">>> Postprocessing predictions...")
    ...
    if verbose: print(">>> Done.", new_pred_label.shape)
    return pred_prob_map, df_pred, new_pred_label


#@title model predict aneurysm
def predict_aneurysm_ver2(image_arr, brain_mask, vessel_16labels, spacing,
                     model2, patch_size=(32, 32, 16), sigma=0.125, n_class=1, overlap=0.75,
                     conf_th=0.1, min_diameter=1, top_k=4, obj_th=0.67, batch_size=600, verbose=False):
    ...
    def sliding_window_inference2(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside", mask=None, batch_size=10, verbose=False, **kwargs):
        ...
        #output = output_sum / tf.clip_by_value(output_weight_sum, 1e-8, tf.reduce_max(output_weight_sum))
        crop_slice = [slice(pad[0], pad[0] + input_x) for pad, input_x in zip(paddings[1:-1], inputs.shape[1:-1])]
        return output[(slice(0, 1), *crop_slice, slice(0, n_class))], count_w
    ...
    if verbose: print(">>> Running sliding window inference...")
    pred_prob_map, count_w = sliding_window_inference2(
        inputs=custom_normalize_1(image_iso)[np.newaxis, ..., np.newaxis],
        roi_size=patch_size, model=model2, overlap=overlap, n_class=n_class,
        importance_map=importance_map, mask=vessel_iso[np.newaxis, ..., np.newaxis],
        batch_size=batch_size,
    ...
    pred_prob_map = pred_prob_map[0, ..., 0].numpy() * vessel_iso
    #print('count:', count_w,' np.max(pred_prob_map):', np.max(pred_prob_map), ' np.mean(pred_prob_map):', np.mean(pred_prob_map))
    pred_prob_map = resize_volume(pred_prob_map, target_size=image_arr.shape, dtype='float32')
...
def model_predict_aneurysm(path_code, path_process, case_name, path_log, gpu_n):
    ...
    try:
        ...
        if gpumRate < 0.6 :
            ...
            print(f"[Done load AI model... ] spend {time.time() - start_load:.0f} sec")

            # 2 Get brain mask，先全照君彥pipeline，來不及拉!!!
            start_synthseg = time.time()
            brain_seg = get_brain_seg(image_arr, spacing, model0)
            brain_bottom_idx = np.where(np.any(brain_seg > 0, axis=(0,1)))[0][0]
            print(f"[Done SynthSEG... ] spend {time.time() - start_synthseg:.0f} sec")
            ...
            del brain_seg, bet_brain_mask, brain_bottom_idx

            #這邊存出brain mask，可以加速後面的流程
            np.save(os.path.join(path_process, 'brain_mask.npy'), brain_mask)

            #讀取2個mask
            # brain_mask = np.load(os.path.join(path_process, 'brain_mask.npy'))        
            # vessel_mask = np.load(os.path.join(path_process, 'vessel_mask.npy'))  
                
            # 3 Get vessel mask
            start_vessel = time.time()
            ...
            print(f"[Done Get vessel mask... ] spend {time.time() - start_vessel:.0f} sec")
            del vessel_threshold, vessel_seed, pred_vessel_mask

            #這邊存出vessel_mask，可以加速後面的流程
            np.save(os.path.join(path_process, 'vessel_mask.npy'), vessel_mask)    

            # 3.5 Get vessel skeleton
            start_skeleton = time.time()
            skeleton_labels = get_vessel_skeleton_labels(vessel_mask, spacing)
            print(f"[Done Get vessel skeleton... ] spend {time.time() - start_skeleton:.0f} sec")

            # 4 get vessel 16labels
            start_vessel16 = time.time()
            vessel_16labels = predict_vessel_16labels(vessel_mask, model3, spacing, verbose=verbose)
            #這邊存出vessel_16labels，可以加速後面的流程
            np.save(os.path.join(path_process, 'vessel_16labels.npy'), vessel_16labels)   
            print(f"[Done get vessel 16labels... ] spend {time.time() - start_vessel16:.0f} sec")   

            #讀取1個mask
            # vessel_16labels = np.load(os.path.join(path_process, 'vessel_16labels.npy')) 

            # 5 Pred aneurysm
            start_pred = time.time()
            pred_prob_map, df_pred, pred_label = predict_aneurysm_ver2(image_arr, brain_mask, vessel_mask, spacing, model2, verbose=verbose)
            print(f"[Done Pred aneurysm... ] spend {time.time() - start_pred:.0f} sec")

            # 6 Save results
            #def save_nii(path_process, brain_mask, pred_vessel, vessel_16labels, pred_label, pred_prob_map, out_dir='test_label_out', verbose=False):
            start_save = time.time()
            save_nii(path_process, brain_mask, vessel_mask, vessel_16labels, pred_label, pred_prob_map, out_dir=path_process)
            print(f"[Done Save results... ] spend {time.time() - start_save:.0f} sec")
