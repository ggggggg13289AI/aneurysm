...
這一版執行前後比較，工作流程:
1. 收到ID後從備份資料中找出是否有上一次的執行結果，有的話就在病人最新一次執行的資料夾開始
2. 將前一次日期的跟最新日期的影像都做SynthSeg(torch版本)，之後將前一次日期的影像跟標註對到最新日期的影像上
3. 後續根據標註的重疊性開始進行追蹤，預期除了原本的aneurysm json欄位外，額外在新增追蹤的json欄位，分3種type:新增、消失、皆有。新增欄位在皆有時會
   填值，填長度是放大還是縮小多少
...
def pipeline_aneurysm_track(ID, 
                            path_output,
    ...
    logging.info('!!! Aneurysm Track call.')
    ...
    print(ID, ' Start...')
    print('step1: find previous execution result')
    logging.info(ID + ' Start...')

    #依照不同情境拆分try需要小心的事項 <= 重要
    try:
        IDSdates = sorted(os.listdir(path_processModel)) #找出所有資料夾
        dict_IDs = organize_data([ID], IDSdates)
        # 輸出結果
        print(dict_IDs[ID])
...
if __name__ == '__main__':
    ...
    pipeline_aneurysm_track(ID, path_output, path_code, path_processModel, path_json, path_log, gpu_n)
