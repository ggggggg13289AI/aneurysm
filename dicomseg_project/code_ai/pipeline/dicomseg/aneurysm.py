...
"""
import sys
sys.path.append('/data/4TB1/pipeline/chuan/code/dicomseg_project')
import json
import os
import pathlib
...
from code_ai.pipeline.dicomseg.schema.aneurysm import AneurysmAITeamRequest, SortedRequest, AneurysmStudyModelRequest, AneurysmStudyRequest,AneurysmAITeamRequest
...
class AneurysmPlatformJSONBuilder(PlatformJSONBuilder[AneurysmAITeamRequest]):
    ...
    def set_study(self, source_images: List[Union[FileDataset, DicomDir]],
        ...
        self._study_request = AneurysmStudyRequest.model_validate(study_dict)
    ...
    def build_study_model(self, pred_json_list: List[Dict[str, Any]]
        ...
        if self.series_type:
            ...
            study_model = dict(lession=len(pred_json_list),
                               ...
                               status="1",
                               report="", )
        return [AneurysmStudyModelRequest.model_validate(study_model)]
...
def main(_id, path_root, group_id):
    """
    ...
    #_id = "00719692_20250522_MR_21405210094"
    #path_root = pathlib.Path('/data/4TB1/pipeline/chuan/process/Deep_Aneurysm').joinpath(_id)
