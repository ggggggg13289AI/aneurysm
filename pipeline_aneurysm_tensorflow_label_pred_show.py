...
這一版會只會在1個group預測nnU-Net的結果，且會加入label的結果一起上傳到平台上
...
from util_aneurysm import reslice_nifti_label_pred_nobrain, create_MIP_pred, AneurysmPipeline, \
...
from code_ai.pipeline.dicomseg.aneurysm_label_pred_show import main as make_pred_json
...
def pipeline_aneurysm(ID, 
    ...
    try:
        ...
        if gpumRate < 0.6:
            ...
            # 定義要傳入的參數，建立指令
            cmd = [
                   "python", "/data/4TB1/pipeline/chuan/code/gpu_aneurysm_only_nnunet.py",
                   "--path_code", path_code,
            ...
            reslice_nifti_label_pred_nobrain(path_nii_n, path_reslice_n)
            ...
            create_MIP_pred(path_dcm_n, path_reslice_n, path_png, gpu_n, create_label_mip=True)
            ...
            shutil.copy(os.path.join(path_reslice_n, 'MIP_Pitch_label.nii.gz'), os.path.join(path_nnunet, 'MIP_Pitch_label.nii.gz'))
            shutil.copy(os.path.join(path_reslice_n, 'MIP_Yaw_label.nii.gz'), os.path.join(path_nnunet, 'MIP_Yaw_label.nii.gz'))
            ...
            make_pred_json(ID, pathlib.Path(path_nnunet), group_id2)  #(_id, path_root, group_id)
...
if __name__ == '__main__':
    ...
    path_process = '/data/4TB1/pipeline/chuan/process_model_unetcls5b1to1shhfemarsna/'  #前處理dicom路徑(test case)
