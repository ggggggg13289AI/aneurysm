...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.oversample_foreground_percent_val = 0.2 #因為
        self.num_iterations_per_epoch = int(250 * 1200 / self.configuration_manager.batch_size) #250
        ...
        self.cls_loss = None  # -> self.initialize
    ...
    def initialize_network_look(self):
        ...
        return self.plans_manager, self.dataset_json, self.configuration_manager, self.num_input_channels, self.network
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.dice_loss0 = self._build_dice_loss0() #正確賦值
            self.dice_loss1 = self._build_dice_loss1() #正確賦值
            self.dice_loss2 = self._build_dice_loss2() #正確賦值
            self.dice_loss3 = self._build_dice_loss3() #正確賦值
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            ...
            print('nnUNet-long-BigBatch-cosine-classifier')
            print('Loss: DC_and_CE_loss')
            print('DC_and_CE_loss => NewDC_loss_and_CE_loss')   
            print('combnine FEMH!!! and Large Big Batch Size 1200!!!')   
    ...
    #建立拆分的loss => 用於classification
    def _build_cls_loss(self):           
        print('Loss: CLS_loss')        
        return CE_loss({}, weight_ce=1, ignore_label=self.label_manager.ignore_label)       
    ...
    def plot_network_architecture(self):
        if self.local_rank == 0:
            try:
                ...
                txt_path = '/data/chuan/nnUNet/nnUNet_results/Dataset077_DeepAneurysm/nnUNetTrainer__nnUNetPlans__3d_fullres/model_summary_classifier-' + str(len(self.configuration_manager.n_conv_per_stage_encoder)) + 'L.txt'
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if isinstance(target, list):
            ...
            #positive = [i.to(self.device, non_blocking=True) for i in positive]
        else:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output_seg, output_cls = self.network(data) #output => target, positive
            # del data
            seg_l = self.loss(output_seg, target)
            cls_l = self.cls_loss(output_cls, positive)
            ce_l = self.ce_loss(output_seg, target)
            dice_l = self.dice_loss(output_seg, target)
            dice_l0 = self.dice_loss0(output_seg, target)
            dice_l1 = self.dice_loss1(output_seg, target)
            dice_l2 = self.dice_loss2(output_seg, target)
            dice_l3 = self.dice_loss3(output_seg, target)
            l = seg_l + cls_l
        ...
        # 轉成 class 預測結果 => 小數點有0.0001的誤差
        # print('cls_l:', cls_l)
        # loss_ce = nn.CrossEntropyLoss()
        # cls_l_test = loss_ce(output_cls, positive.squeeze(1).long())
        # print('cls_l_test:', cls_l_test)

        #print('output_seg.shape:', output_seg[0].shape) #=> torch.Size([1200, 2])
        #print('output_cls.shape:', output_cls.shape) #=> torch.Size([1200, 2])
        preds = torch.argmax(output_cls, dim=1)
        positive = positive.squeeze(1).long()  # 把 shape [1200, 1] -> [1200]
        #print('preds.shape:', preds.shape) #=> [1200] 合理
        #print('positive.shape:', positive.shape) #=> torch.Size([1200, 1])

        # 計算 TP, TN, FP, FN
        TP = ((preds == 1) & (positive == 1)).sum().item()
        TN = ((preds == 0) & (positive == 0)).sum().item()
        FP = ((preds == 1) & (positive == 0)).sum().item()
        FN = ((preds == 0) & (positive == 1)).sum().item()

        # 計算 metrics
        accuracy = (TP + TN) / (TP + TN + FP + FN + 1e-8)
        sensitivity = TP / (TP + FN + 1e-8)   # Recall for positive
        specificity = TN / (TN + FP + 1e-8)   # Recall for negative
        ...
        #         }
        return {'loss': l.detach().cpu().numpy(), 'seg_loss': seg_l.detach().cpu().numpy(), 'cls_loss': cls_l.detach().cpu().numpy(), 'ce_loss': ce_l.detach().cpu().numpy(), 'dice_loss': dice_l.detach().cpu().numpy(), 
                'tp_hard': tp_hard.cpu().numpy(), 'fp_hard': fp_hard.cpu().numpy(), 'fn_hard': fn_hard.cpu().numpy(),
                'dice_loss0': dice_l0.detach().cpu().numpy(), 'dice_loss1': dice_l1.detach().cpu().numpy(), 'dice_loss2': dice_l2.detach().cpu().numpy(), 'dice_loss3': dice_l3.detach().cpu().numpy(),
                'dc0': metrics['dc0'], 'dc1': metrics['dc1'], 'dc2': metrics['dc2'], 'dc3': metrics['dc3'], 'fake_dcl': fake_dcl,
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            seg_losses = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(seg_losses, outputs['seg_loss'])
            seg_loss_here = np.vstack(seg_losses).mean()

            cls_losses = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(cls_losses, outputs['cls_loss'])
            cls_loss_here = np.vstack(cls_losses).mean()
            ...
            dice_losses0 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses0, outputs['dice_loss0'])
            dice_loss0_here = np.vstack(dice_losses0).mean()

            dice_losses1 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses1, outputs['dice_loss1'])
            dice_loss1_here = np.vstack(dice_losses1).mean()

            dice_losses2 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses2, outputs['dice_loss2'])
            dice_loss2_here = np.vstack(dice_losses2).mean()

            dice_losses3 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses3, outputs['dice_loss3'])
            dice_loss3_here = np.vstack(dice_losses3).mean()

            dcs0 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs0, outputs['dc0'])
            dice0_here = np.vstack(dcs0).mean()  

            dcs1 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs1, outputs['dc1'])
            dice1_here = np.vstack(dcs1).mean()  

            dcs2 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs2, outputs['dc2'])
            dice2_here = np.vstack(dcs2).mean() 

            dcs3 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs3, outputs['dc3'])
            dice3_here = np.vstack(dcs3).mean() 
            ...
            accuracys = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(accuracys, outputs['accuracy'])
            accuracy_here = np.vstack(accuracys).mean() 

            sensitivitys = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(sensitivitys, outputs['sensitivity'])
            sensitivity_here = np.vstack(sensitivitys).mean() 

            specificitys = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(specificitys, outputs['specificity'])
            specificity_here = np.vstack(specificitys).mean()             
            ...
            dice0_here = np.mean(outputs['dc0'])
            dice1_here = np.mean(outputs['dc1'])
            dice2_here = np.mean(outputs['dc2'])
            dice3_here = np.mean(outputs['dc3'])
            fake_dice_loss_here = np.mean(outputs['fake_dcl'])
            dice_loss0_here = np.mean(outputs['dice_loss0'])
            dice_loss1_here = np.mean(outputs['dice_loss1'])
            dice_loss2_here = np.mean(outputs['dice_loss2'])
            dice_loss3_here = np.mean(outputs['dice_loss3'])
        ...
        self.logger.log('train_dice_loss0', dice_loss0_here, self.current_epoch)
        self.logger.log('train_dice_loss1', dice_loss1_here, self.current_epoch)
        self.logger.log('train_dice_loss2', dice_loss2_here, self.current_epoch)
        self.logger.log('train_dice_loss3', dice_loss3_here, self.current_epoch)
        self.logger.log('train_mean_fg_dice', mean_fg_dice, self.current_epoch)
        self.logger.log('train_supervision_dice0', dice0_here, self.current_epoch)
        self.logger.log('train_supervision_dice1', dice1_here, self.current_epoch)
        self.logger.log('train_supervision_dice2', dice2_here, self.current_epoch) 
        self.logger.log('train_supervision_dice3', dice3_here, self.current_epoch)        
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            seg_losses = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(seg_losses, outputs_collated['seg_loss'])
            seg_loss_here = np.vstack(seg_losses).mean()

            cls_losses = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(cls_losses, outputs_collated['cls_loss'])
            cls_loss_here = np.vstack(cls_losses).mean()
            ...
            dice_losses0 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses0, outputs_collated['dice_loss0'])
            dice_loss0_here = np.vstack(dice_losses0).mean()

            dice_losses1 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses1, outputs_collated['dice_loss1'])
            dice_loss1_here = np.vstack(dice_losses1).mean()

            dice_losses2 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses2, outputs_collated['dice_loss2'])
            dice_loss2_here = np.vstack(dice_losses2).mean()

            dice_losses3 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dice_losses3, outputs_collated['dice_loss3'])
            dice_loss3_here = np.vstack(dice_losses3).mean()

            dcs0 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs0, outputs_collated['dc0'])
            dice0_here = np.vstack(dcs0).mean()  

            dcs1 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs1, outputs_collated['dc1'])
            dice1_here = np.vstack(dcs1).mean()  

            dcs2 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs2, outputs_collated['dc2'])
            dice2_here = np.vstack(dcs2).mean() 

            dcs3 = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(dcs3, outputs_collated['dc3'])
            dice3_here = np.vstack(dcs3).mean() 
            ...
            dice_loss0_here = np.mean(outputs_collated['dice_loss0'])
            dice_loss1_here = np.mean(outputs_collated['dice_loss1'])
            dice_loss2_here = np.mean(outputs_collated['dice_loss2'])
            dice_loss3_here = np.mean(outputs_collated['dice_loss3'])
            dice0_here = np.mean(outputs_collated['dc0'])
            dice1_here = np.mean(outputs_collated['dc1'])
            dice2_here = np.mean(outputs_collated['dc2'])
            dice3_here = np.mean(outputs_collated['dc3'])
        ...
        self.logger.log('val_dice_loss0', dice_loss0_here, self.current_epoch)
        self.logger.log('val_dice_loss1', dice_loss1_here, self.current_epoch)
        self.logger.log('val_dice_loss2', dice_loss2_here, self.current_epoch)
        self.logger.log('val_dice_loss3', dice_loss3_here, self.current_epoch)
        self.logger.log('val_supervision_dice0', dice0_here, self.current_epoch)
        self.logger.log('val_supervision_dice1', dice1_here, self.current_epoch)
        self.logger.log('val_supervision_dice2', dice2_here, self.current_epoch) 
        self.logger.log('val_supervision_dice3', dice3_here, self.current_epoch)  
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('train_dice_loss0', np.round(self.logger.my_fantastic_logging['train_dice_loss0'][-1], decimals=4))
        self.print_to_log_file('train_dice_loss1', np.round(self.logger.my_fantastic_logging['train_dice_loss1'][-1], decimals=4))
        self.print_to_log_file('train_dice_loss2', np.round(self.logger.my_fantastic_logging['train_dice_loss2'][-1], decimals=4))
        self.print_to_log_file('train_dice_loss3', np.round(self.logger.my_fantastic_logging['train_dice_loss3'][-1], decimals=4))
        ...
        self.print_to_log_file('val_dice_loss0', np.round(self.logger.my_fantastic_logging['val_dice_loss0'][-1], decimals=4))
        self.print_to_log_file('val_dice_loss1', np.round(self.logger.my_fantastic_logging['val_dice_loss1'][-1], decimals=4))
        self.print_to_log_file('val_dice_loss2', np.round(self.logger.my_fantastic_logging['val_dice_loss2'][-1], decimals=4))
        self.print_to_log_file('val_dice_loss3', np.round(self.logger.my_fantastic_logging['val_dice_loss3'][-1], decimals=4))
        ...
        self.print_to_log_file('train deep_supervision dice0', np.round(self.logger.my_fantastic_logging['train_supervision_dice0'][-1], decimals=4))   
        self.print_to_log_file('train deep_supervision dice1', np.round(self.logger.my_fantastic_logging['train_supervision_dice1'][-1], decimals=4))   
        self.print_to_log_file('train deep_supervision dice2', np.round(self.logger.my_fantastic_logging['train_supervision_dice2'][-1], decimals=4))
        self.print_to_log_file('train deep_supervision dice3', np.round(self.logger.my_fantastic_logging['train_supervision_dice3'][-1], decimals=4))   
        self.print_to_log_file('val Pseudo dice', [np.round(i, decimals=4) for i in
                                               self.logger.my_fantastic_logging['dice_per_class_or_region'][-1]])
        self.print_to_log_file('val deep_supervision dice0', np.round(self.logger.my_fantastic_logging['val_supervision_dice0'][-1], decimals=4))     
        self.print_to_log_file('val deep_supervision dice1', np.round(self.logger.my_fantastic_logging['val_supervision_dice1'][-1], decimals=4))        
        self.print_to_log_file('val deep_supervision dice2', np.round(self.logger.my_fantastic_logging['val_supervision_dice2'][-1], decimals=4))         
        self.print_to_log_file('val deep_supervision dice3', np.round(self.logger.my_fantastic_logging['val_supervision_dice3'][-1], decimals=4))   

        self.print_to_log_file('train_accuracy', np.round(self.logger.my_fantastic_logging['train_accuracys'][-1], decimals=4))
        self.print_to_log_file('train_sensitivity', np.round(self.logger.my_fantastic_logging['train_sensitivitys'][-1], decimals=4))
        self.print_to_log_file('train_specificity', np.round(self.logger.my_fantastic_logging['train_specificitys'][-1], decimals=4))
        self.print_to_log_file('val_accuracy', np.round(self.logger.my_fantastic_logging['val_accuracys'][-1], decimals=4))
        self.print_to_log_file('val_sensitivity', np.round(self.logger.my_fantastic_logging['val_sensitivitys'][-1], decimals=4))
        self.print_to_log_file('val_specificity', np.round(self.logger.my_fantastic_logging['val_specificitys'][-1], decimals=4))
        self.print_to_log_file(
        ...
        mlflow.log_metric("train_seg_loss", np.round(self.logger.my_fantastic_logging['train_seg_losses'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train_cls_loss", np.round(self.logger.my_fantastic_logging['train_cls_losses'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        ...
        mlflow.log_metric("train_dice_loss0", np.round(self.logger.my_fantastic_logging['train_dice_loss0'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train_dice_loss1", np.round(self.logger.my_fantastic_logging['train_dice_loss1'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train_dice_loss2", np.round(self.logger.my_fantastic_logging['train_dice_loss2'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train_dice_loss3", np.round(self.logger.my_fantastic_logging['train_dice_loss3'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        ...
        mlflow.log_metric("val_seg_loss", np.round(self.logger.my_fantastic_logging['val_seg_losses'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val_cls_loss", np.round(self.logger.my_fantastic_logging['val_cls_losses'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        ...
        mlflow.log_metric("val_dice_loss0", np.round(self.logger.my_fantastic_logging['val_dice_loss0'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val_dice_loss1", np.round(self.logger.my_fantastic_logging['val_dice_loss1'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val_dice_loss2", np.round(self.logger.my_fantastic_logging['val_dice_loss2'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val_dice_loss3", np.round(self.logger.my_fantastic_logging['val_dice_loss3'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        ...
        mlflow.log_metric("train deep_supervision dice0", np.round(self.logger.my_fantastic_logging['train_supervision_dice0'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train deep_supervision dice1", np.round(self.logger.my_fantastic_logging['train_supervision_dice1'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train deep_supervision dice2", np.round(self.logger.my_fantastic_logging['train_supervision_dice2'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train deep_supervision dice3", np.round(self.logger.my_fantastic_logging['train_supervision_dice3'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val deep_supervision dice0", np.round(self.logger.my_fantastic_logging['val_supervision_dice0'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val deep_supervision dice1", np.round(self.logger.my_fantastic_logging['val_supervision_dice1'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val deep_supervision dice2", np.round(self.logger.my_fantastic_logging['val_supervision_dice2'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("val deep_supervision dice3", np.round(self.logger.my_fantastic_logging['val_supervision_dice3'][-1], decimals=4), step=self.current_epoch) # metric 是可以持續被更新的.
        mlflow.log_metric("train_accuracy", np.round(self.logger.my_fantastic_logging['train_accuracys'][-1], decimals=4), step=self.current_epoch)
        mlflow.log_metric("train_sensitivity", np.round(self.logger.my_fantastic_logging['train_sensitivitys'][-1], decimals=4), step=self.current_epoch)
        mlflow.log_metric("train_specificity", np.round(self.logger.my_fantastic_logging['train_specificitys'][-1], decimals=4), step=self.current_epoch)
        mlflow.log_metric("val_accuracy", np.round(self.logger.my_fantastic_logging['val_accuracys'][-1], decimals=4), step=self.current_epoch)
        mlflow.log_metric("val_sensitivity", np.round(self.logger.my_fantastic_logging['val_sensitivitys'][-1], decimals=4), step=self.current_epoch)
        mlflow.log_metric("val_specificity", np.round(self.logger.my_fantastic_logging['val_specificitys'][-1], decimals=4), step=self.current_epoch)
    ...
    def run_training(self):
        ...
        run_name =  "nnResUNet-long-BigBatch-cosine-classifier-AneDilate-1to1"
        print(run_name)
    ...
    #用來看資料變數叫什麼名稱
    def run_training_ShowTraingData(self):
        self.on_train_start()
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                if isinstance(batch, dict):
                    print("Batch keys:", batch.keys()) #Batch keys: dict_keys(['data', 'properties', 'positives', 'keys', 'target'])
