...
def predict_aneurysm_old(image_arr, brain_mask, vessel_16labels, spacing,
...
def predict_aneurysm(image_arr, brain_mask, vessel_16labels, spacing,
    ...
    @tf.function
    def run_model2(x, model2, importance_map):
        pred = model2.serve(x)
        return pred * importance_map
    ...
    def sliding_window_inference2(inputs, roi_size, model, overlap, n_class, importance_map, strategy="overlap_inside", mask=None, batch_size=10, verbose=False, **kwargs):
        ...
        for i, window_slice in enumerate(window_slices):
            start, size = window_slice
            if (mask is None) or tf.math.reduce_any(tf.slice(mask_padded, begin=start, size=size)):
                patch = tf.slice(input_padded, begin=start, size=size)  # shape: (1, D, H, W, C)
                patch = tf.squeeze(patch, axis=0)  # shape: (D, H, W, C)
            ...
            if len(batch_inputs) == batch_size or i == len(window_slices) - 1:
                ...
                for j, (s, sz) in enumerate(batch_indices):
                    ...
                    # Calculate actual shape after slice to construct padding
                    # actual_shape = tf.shape(pred)
                    # pad_before = [s[d] for d in range(3)]
                    # pad_after = [output_shape[d + 1] - pad_before[d] - actual_shape[d] for d in range(3)]
                    # Ensure non-negative padding
                    # pad_after = [max(0, p) for p in pad_after]
                    # pad = [[0, 0]] + [[pad_before[d], pad_after[d]] for d in range(3)] + [[0, 0]]
                    ...
                    try:
                        # pred_padded = tf.pad(pred[tf.newaxis], pad)
                        # weight_padded = tf.pad(importance_map, pad)
...
def model_predict_aneurysm(path_code, path_process, case_name, path_log, gpu_n):
    ...
    try:
        ...
        if gpumRate < 0.6 :
            ...
            #這邊存出brain mask，可以加速後面的流程
            # np.save(os.path.join(path_process, 'vessel_mask.npy'), brain_mask)   
