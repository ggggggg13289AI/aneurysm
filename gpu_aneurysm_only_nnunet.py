...
#@title save nifti，這邊是用我的
def save_nii_preprocess_image(path_process, image_arr, out_dir='test_label_out', verbose=False):
    ## load original image
    #建立分別放正規化影像與vessel的資料夾
    if not os.path.isdir(os.path.join(out_dir, 'Normalized_Image')):
        os.mkdir(os.path.join(out_dir, 'Normalized_Image'))
    ...
    ## vessel用複製的
    if not os.path.isdir(os.path.join(out_dir, 'Vessel', 'DeepAneurysm_00001_0000.nii.gz')):
        shutil.copy(os.path.join(path_process, 'Vessel.nii.gz'), os.path.join(out_dir, 'Vessel', 'DeepAneurysm_00001_0000.nii.gz'))
        print("[save] --->", os.path.join(out_dir, 'Vessel', 'DeepAneurysm_00001_0000.nii.gz'))
    return 
...
def model_predict_aneurysm(path_code, path_process, case_name, path_log, gpu_n):
    ...
    try:
        ...
        if gpumRate < 0.6 :
            ...
            # 5.1 先存出正規化的影像跟血管
            save_nii_preprocess_image(path_process, image_arr, out_dir=path_process) 
            ...
            predict_from_raw_data(path_normimg,
                                  path_vessel,
                                  path_process,
                                  '/data/4TB1/pipeline/chuan/code/nnUNet/nnUNet_results/Dataset121_DeepAneurysm/nnUNetTrainer__nnUNetPlans__3d_fullres',
                                  (8,),
                                  0.25,
                                  use_gaussian=True,
                                  use_mirroring=False,
                                  ...
                                  plans_json_name='nnUNetPlans_classifier_64x-5L-b900.json',  # 可以根據需要修改json檔名
                                  has_classifier_output=True,  # 如果模型有classifier輸出則設為True
            ...
            pred_prob_map, df_pred, new_pred_label = filter_aneurysm(prob, spacing_nn, conf_th=0.1, min_diameter=2, top_k=4, obj_th=0.45)
